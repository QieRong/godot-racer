...
- [x] **真实碰撞体取证**（2026-09-23）：加 `_probe_collision_geometry()`，用物理世界里的真实
      ...
      · 车速 0、`engine_force = 0`、`probe_ahead_m() = 3.36m` → 限速被压到约 3.3 km/h，
        **是 AI 自己踩死刹车**；动态路障此时在 300m 外，与卡死无关。
- [x] 现状：`-Level 3` 已能跑完 **1 圈**（114.2s / 13591 帧），但**自救 5 次** → 验收仍判 ✘。
- [ ] **真正的修复**：先查 `probe_ahead_m()` 的探测盒几何（它把侧前方实体算成了正前方障碍），
      而不是先动关卡几何 —— 现有证据不支持改石头位置或尺寸。
