...
| `layout` | 路段 DSL + 高度剖面 DSL：**53 条解析/闭环/曲率/剖面断言**（合法串、残留 token、参数个数、NaN、chicane 无解、净转角不对、直道平行无法闭环、半径过小… / 剖面：严格递增、首尾 t、首尾同高、高度下限、与长度无关的量级闸门）+ 本关几何体检（闭环 ≤0.1m、最小弯半径 ≥ 关卡下限、每 2m 航向步长 ≤8°、曲率跳变 ≤0.07/m、内护栏不自交、反查唯一性）。**防假绿**：模块不可用、断言不足 40 条、或剖面用例没跑满 10/10 且全绿，即判失败。⚠ **剖面坡度不在本项判**（`t` 是归一化位置，解析器拿不到赛道长度），由任务 3 的 `apply_elevation` 在最终曲线上量峰值 |
